...
- (void)handleMoveToIndexTick
{
    ...
    // otherwise, calculate how much we should be scrolling our content by
        ...
        if (delta > self.scrollView.frame.size.width * .5 + self.scrollView.frame.size.width * _views.count) {
            delta -= self.scrollView.frame.size.width * _views.count;
        }
...
{
    // remove previous
    for (UIView *vc in _views) {
        [vc removeFromSuperview];
    }
    
    currentIndex = 0;
    _views = [NSMutableArray arrayWithArray:viewControllers];
...
- (void)initFirstViewController
{
    UIView *viewController = [_views objectAtIndex:0];
    [self.scrollView addSubview:viewController];
    viewController.frame = CGRectMake(self.scrollView.frame.size.width, 0, self.scrollView.frame.size.width, self.scrollView.frame.size.height);
...
- (void)initLastViewController
{
    UIView *viewController = [_views lastObject];
    [self.scrollView addSubview:viewController];
    viewController.frame = CGRectMake(self.scrollView.frame.size.width * _views.count, 0, self.scrollView.frame.size.width, self.scrollView.frame.size.height);
...
- (void)initCurrentViewController
{
    UIView *viewController = [_views objectAtIndex:self.currentIndex];
    [self.scrollView addSubview:viewController];
    viewController.frame = CGRectMake(self.scrollView.frame.size.width + self.currentIndex * self.scrollView.frame.size.width, 0, self.scrollView.frame.size.width, self.scrollView.frame.size.height);
...
- (void)loadPrevViewController
{
    UIView *currentVC = [self.views objectAtIndex:self.currentIndex];
    NSInteger prevIndex = [self indexOfPreviousController];
    
    UIView *viewController = [self.views objectAtIndex:prevIndex];
    [self.scrollView addSubview:viewController];
    
    viewController.frame = CGRectMake(currentVC.frame.origin.x - self.scrollView.frame.size.width, 0, self.scrollView.frame.size.width, self.scrollView.frame.size.height);
}
...
- (void)loadNextViewController
{
    ...
    UIView *viewController = [self.views objectAtIndex:nextIndex];
    [self.scrollView addSubview:viewController];
...
- (void)unloadUnusedViewControllers
{
    for (int i=0; i < _views.count; i++) {
        if (i != self.currentIndex && i != [self indexOfPreviousController] && i != [self indexOfNextController]) {
            UIView *vc = [_views objectAtIndex:i];
            if (vc) {
                [vc removeFromSuperview];
                vc = nil;
//                [vc viewDidUnload];
...
- (void)updateContentSize
{
    ...
    // 9, 0, 1 ... 8, 9, 0
    CGFloat wv = (self.views.count + 2) * self.scrollView.frame.size.width;
...
{
    CGFloat offset = fmodf(self.scrollView.contentOffset.x, self.scrollView.frame.size.width) - 1;
    self.scrollView.contentOffset = CGPointMake(self.views.count * self.scrollView.frame.size.width + offset, 0);
